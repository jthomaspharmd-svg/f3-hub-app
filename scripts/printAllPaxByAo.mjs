...
const main = async () => {
  ...
  const { paxByAo } = parsePaxAoTab(paxAoRows, paxById);

  const aoIds = Object.keys(paxByAo).sort();
  console.log(`Loaded AOs (${aoIds.length}): ${aoIds.join(", ")}`);

  for (const aoId of aoIds) {
    const list = paxByAo[aoId] || [];
    console.log(`\n=== ${aoId} (${list.length}) ===`);
